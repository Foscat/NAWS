...
module.exports = {
  ...
  findUserByEmail: function(req,res){
    console.log("Find user by email", req.params.email);
    if(req.params.email){
  ...
  findUserByUsername: function(req,res){
    ...
    if(req.params.username){
      ...
      console.log("Error bro..")
      res.status(204).send("No username in params")
    }
  },
  ...
  // If an admin chooses to remove a collaborator from a board
  // or if a user removes themselves from being a collaborator on a board
  removeColabStatus: function(req,res){
    console.log("Remove collaborator status for user on board", req.params);
    // Find a user by id
    db.User.findById(req.params.userId).then(user=>{
      console.log("User found", user);
      let remIndex;
      // Check all colab boards and remove the one with the matching id
      user.colabBoards.forEach((board, i) => {if(board.id === req.params.boardId)remIndex=i});
      user.colabBoards.splice(remIndex,1);
      // Check updated colabs
      console.log("Colabs", user.colabBoards);
      // Update user colabBoards with edited information
      db.User.findByIdAndUpdate(req.params.userId, {
        colabBoards: user.colabBoards
      }).then(updated => {
        console.log("Updated user", updated);
        // Return the updated version of the user
        res.send({msg:"It worked", info: updated});
      }).catch(err => console.error("update user hit an error",err))
    }).catch(findErr=>console.error("Find user by id error", findErr))
  }
};